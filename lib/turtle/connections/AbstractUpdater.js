...
import { OPAQUE_UINT8ARRAY } from '../codecs/globalCodec.js'
...
let memoryLogTimeout
function logMemory (now) {
  const dateBlock = `\x1b[96m${now.toLocaleString()} \x1b[2m(${now.toISOString()})\x1b[0m`
  const memoryBlock = `\x1b[45;97m : ${Math.round(process.memoryUsage().heapTotal / 1024 / 1024)} MB : \x1b[0m`
  console.log(`${dateBlock} ${memoryBlock}`)
}
...
export function logUpdate (name, publicKey, uint8ArrayAddresses, isIncoming) {
  logDebug(() => {
    ...
    clearTimeout(memoryLogTimeout)
    const now = new Date()
    lastForcedUpdate ||= now
    if (memoryLogTimeout && now - lastForcedUpdate > 10000) {
      lastForcedUpdate = now
      logMemory(now)
    }
    memoryLogTimeout = setTimeout(() => {
      lastForcedUpdate = 0
      memoryLogTimeout = null
      logMemory(new Date())
    }, 500)
  })
}
