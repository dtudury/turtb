 ...
 * @property {Array.<string>} optionLabels
...
export class CompositeCodec extends Addressifier {
  ...
  codecsByName = Object.fromEntries(Object.entries({
    ...
    Uint8Array_short: {
      options: 5,
      optionLabels: range(5).map(n => `(inline ${n}-byte array)`),
    ...
    Uint8Array_long: {
      ...
      inspect: (code) => {
        const option = this.#codeToOption(code)
        const basic = this.#basicInspect(code, [option])
        return basic
      }
    },
    string: {
      ...
      decode: (code) => {
        const option = this.#codeToOption(code)
        const [v] = this.#readParts(code, [option])
        return new TextDecoder().decode(this.decode(v.code))
      },
      encode: (string) => {
    ...
    float64: {
      ...
      decode: (code) => {
        const option = this.#codeToOption(code)
        const [v] = this.#readParts(code, [option])
        return new Float64Array(this.decode(v.code).buffer)[0]
      },
      encode: (float64) => {
    ...
    nameValuePair: {
      options: 25,
      optionLabels: range(25).map(w => `(${optionLabels[Math.floor(w / 5)]}, ${optionLabels[w % 5]})`),
      getWidth: (code) => {
        const option = this.#codeToOption(code)
      ...
      decode: (code) => {
        ...
        const option1 = option % 5
        const [v0, v1] = this.#readParts(code, [option0, option1])
        return [this.decode(v0.code), this.decode(v1.code)]
      },
      encode: () => undefined,
      _encode: (duple, refsOptions) => {
        ...
        const footer = this.codecsByName.nameValuePair.baseFooter + option0 * 5 + option1
      ...
      inspect: (code) => {
        ...
        const option0 = Math.floor(option / 5)
        const option1 = option % 5
        const basic = this.#basicInspect(code, [option0, option1])
        return basic
      }
    },
    duple: {
    ...
    object: {
      ...
      decode: (code) => {
        const option = this.#codeToOption(code)
        const [v] = this.#readParts(code, [option])
        const flatDuples = this.decode(v.code).flatDuples()
        console.log({ v, flatDuples })
  ...
  #basicInspect = (code, partsOptions) => {
    ...
    const basic = {
      type: codec.type,
      optionLabel: codec?.optionLabels?.[option],
    ...
    if (partsOptions) {
      basic.parts = this.#readParts(code, partsOptions).map(part => {
...
const optionLabels = range(5).map(option => option ? `(${option}-byte address)` : '(inline encoded value)')
