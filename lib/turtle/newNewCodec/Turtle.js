 ...
 * @property {function(): any} getDecoded
 */
...
export class Turtle extends Addressifier {
  ...
  #decorateCodecEntries = ([type, codec]) => {
    codec.type = type
  ...
  #codecsByName = Object.fromEntries(Object.entries({
    ...
    UINT7: {
      ...
      decode: (code) => {
        return this.#decodeParts(code)[0].getDecoded()
      },
      encode: (uint7Byte) => uint7Byte instanceof Uint8Array && uint7Byte.length === 1 && uint7Byte[0] < 128 && new Uint8Array([this.#codecsByName.UINT7.baseFooter + uint7Byte[0]])
    },
    // UINT7: {
    //   partReaders: [this.#numberPart],
    //   decode: (code) => {
    ...
    COMMIT: {
      partReaders: [this.#wordPart],
      decode: (code) => {
        const parts = this.#decodeParts(code)
        const lengthVar = parts[0].getDecoded()
        const length = VarToNumber(lengthVar)
        const uint8Array = code.slice(-1 - lengthVar.length - length, -1 - lengthVar.length)
        console.log({ length, uint8Array, code })
        return new Commit(uint8Array)
      },
      getWidth: (code) => {
        const parts = this.#decodeParts(code)
        const lengthVar = parts[0].getDecoded()
