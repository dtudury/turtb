...
export class Assert {
  ...
  async equal (actual, expected, message, debug = true) {
    ...
    if (!this.runner.upserter) {
      return this.assert(TON.stringify(actual) === TON.stringify(expected), passMessage, failMessage)
    }
  ...
  async ok (valueToCheck, message) {
    return this.assert(valueToCheck, message)
  }

  async deepEqual (actual, expected, message) {
    return this.equal(actual, expected, message)
  }

  async throws (f, message) {
    return this.throw(f, message)
  }
