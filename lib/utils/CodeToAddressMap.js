export class CodeToAddressMap {
  ...
  get (addressOrCode) {
    ...
    if (typeof addressOrCode === 'number') {
      const { uint8Array, byteLength } = this.arrayOfUint8Arrays.getUint8ArrayInfoAt(addressOrCode)
      return uint8Array.subarray(0, uint8Array.length + addressOrCode + 1 - byteLength)
    }
  ...
  set (code) {
    ...
    if (!this.branches[matchingBitCount]) {
      this.branches[matchingBitCount] = new CodeToAddressMap(this.arrayOfUint8Arrays, matchingByteCount)
    }
...
export class ArrayOfUint8Arrays {
   ...
   */
  getUint8ArrayInfoAt (byteIndex) {
    const index = this.byteIndexToIndex(byteIndex)
    return this.uint8ArrayInfos[index]
  }

  byteAt (byteIndex) {
    return atWithin(byteIndex, this.getUint8ArrayInfoAt(byteIndex))
  }

  slice (start, end) {
    const uint8ArrayInfo = this.getUint8ArrayInfoAt(start)
    if (end > uint8ArrayInfo.byteLength) throw new Error('slice must start and end in same array')
    const offset = uint8ArrayInfo.byteLength - uint8ArrayInfo.uint8Array.length
    return uint8ArrayInfo.uint8Array.slice(start - offset, end - offset)
  }
}
