...
export function combineUint8ArrayLikes (uint8ArrayLikes) {
  if (!Array.isArray(uint8ArrayLikes)) throw new Error('combineUint8ArrayLikes accepts an array of Uint8ArrayLikes')
  const uint8Arrays = uint8ArrayLikes.map(uint8ArrayLike => {
    ...
    if (uint8ArrayLike instanceof ArrayBuffer) return new Uint8Array(uint8ArrayLike)
    if (Array.isArray(uint8ArrayLike)) return combineUint8ArrayLikes(uint8ArrayLike)
