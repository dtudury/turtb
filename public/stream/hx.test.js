...
import { hx, HxElement, HxText } from './hx.js'

describe(import.meta.url, ({ test }) => {
  test('plain element', ({ assert }) => {
    const [div] = hx`<div></div>`
    assert.ok(div instanceof HxElement)
  ...
  test('boolean attribute (no value)', ({ assert }) => {
    const [el] = hx`<button disabled></button>`
  ...
  test('self-closing syntax', ({ assert }) => {
    const [el] = hx`<MyComponent />`
    assert.ok(el instanceof HxElement)
